...
base %>% mutate(FechaConstitucion=ymd(FechaConstitucion) ) %>% 
  ...
  ggplot(aes(x=FechaConstitucion,y=sindicatos_constituidos,color=as.factor(Region)))+geom_line()+
  facet_wrap(~as.factor(Region),scales = "free")+
  theme_bw() +
    scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) + 
    labs(subtitle = "Sindicatos constituidos cada mes por región",
       x="Tiempo", 
       y = "Sindicatos",
       caption = "Repositorio de Estadísticas Sindicales (RES)") + 
  geom_vline(xintercept = ymd("2019-10-01"),linetype = "dashed", colour = "red") +
  geom_vline(xintercept = ymd("2020-03-01"),linetype = "dashed", colour = "red") + 
